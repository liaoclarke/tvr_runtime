...
#include <tvr/Common/ConfigByteSwapping.h>
...
#if defined(TVR_HAVE_INTRIN_H) && defined(TVR_HAVE_WORKING_MS_BYTESWAPS)
#include <intrin.h>
#endif

#ifdef TVR_HAVE_BYTESWAP_H
#include <byteswap.h>
#endif
...
#if defined(TVR_HAVE_WORKING_MS_BYTESWAPS)
...
#elif defined(TVR_HAVE_BYTESWAP_H) && defined(TVR_HAVE_WORKING_BSWAP)
        template <> struct ByteSwap<uint16_t> {
...
#elif defined(TVR_HAVE_BYTESWAP_H) &&                                         \
    defined(TVR_HAVE_WORKING_BSWAP_UNDERSCORE)
        template <> struct ByteSwap<uint16_t> {
...
#elif defined(TVR_HAVE_BYTESWAP_H) &&                                         \
    defined(TVR_HAVE_WORKING_UNDERSCORES_BSWAP)
        template <> struct ByteSwap<uint16_t> {
