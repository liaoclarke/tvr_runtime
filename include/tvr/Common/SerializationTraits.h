#ifndef INC_Common_SerializationTraits_h
#define INC_Common_SerializationTraits_h

// Internal Includes
#include <tvr/Common/AlignmentPadding.h>
#include <tvr/Common/Endianness.h>
#include <tvr/Common/SerializationTags.h>
#include <tvr/Util/BoolC.h>
#include <tvr/Util/Vec2C.h>
#include <tvr/Util/Vec3C.h>
#include <tvr/Util/TypeSafeId.h>
...
namespace common {
    ...
    namespace serialization {
        ...
        struct SerializationTraits {
            ...
            /// src/tvr/common/SerializationTraitExample.h for a starting
            /// point.
            ///
        ...
        /// which we'll stick in uint8_t (TVR_CBool) types. Note that if you're
        ...
        struct SerializationTraits<DefaultSerializationTag<bool>, void>
            ...
            static void serialize(BufferType &buf, Base::param_type val,
                                  tag_type const &) {
                serializeRaw(
                    buf, static_cast<TVR_CBool>(val ? TVR_TRUE : TVR_FALSE));
            }
            template <typename BufferReaderType>
        ...
        template <>
        struct SimpleStructSerialization<TVR_Vec2>
        ...
        template <>
        struct SimpleStructSerialization<TVR_Vec3>
            : SimpleStructSerializationBase {
            template <typename F, typename T> static void apply(F &f, T &val) {
                f(val.data[0]);
                f(val.data[1]);
